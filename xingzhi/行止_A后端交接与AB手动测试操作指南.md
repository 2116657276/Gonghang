...
更新：2026-09-17 · 工作分支：`jianlin` · 远程发布目标：`main` · 适用：消费者新月度预算产品

> 本文按**当前代码**写，不把规划文档中的未来接口冒充已上线接口。文中金额单位是“分”：`8000` = ¥80，`50000` = ¥500。手动写入只在隔离的本地测试数据库进行；不要用已有历史订单／沙盒凭据所在的数据库做破坏性试验。

当前前端开工入口是[开发计划](docs/03-engineering/development-plan.md)与[小程序实施方案](docs/03-engineering/miniapp-frontend.md)。本文的 HTTP 手测使用现有 Cookie／Origin，不是 Taro 请求示例；小程序会话增量尚待 M2.1 实施。第 3—4 节保留 A00 consumer-a 与固定 CONSUMER-DINNER-99 的手测组合，不能与新隔离场景账号／商品混用。小程序日常联调用新场景标识创建样例，账号及对象 ID 取初始化输出。

## 1. 先回答：A 是不是写完了？

按当前 A00—A06 和 B00—B07 分工，核心领域实现与后端补充收口均已完成，HTTP 集中验收已通过，M1 后端已放行进入 M2。A00 Demo 资金样例提供独立初始化入口；A01 本人账户事实/撤回、A02 自然月预算/项目/目标、A03 逐日现金流、A04 报价和意外预览、A05 同事务最终准入、A06 受控资金事件与月度复盘，以及 B03—B07 购买、Worker、调整、退款恢复和证据读取均已有实现。A02 的项目写函数和 A03 的草稿评估已由 `buildApp` 默认接入 B 路由；当前迁移为 001—029，集中验收套件 74/74，同日后续补充回归 75/75（见验证策略第十四节，构建及提交登记也已完成）；服务端与共享契约类型检查通过。

这不是“整个新版产品已完成”。M1 已有证据覆盖受控 simulation 领域主链和补充收口集中验收（见[新版验证策略](docs/04-quality/verification.md)第十四节）；真实银行账户持续同步、新消费者流程的支付宝 sandbox 连续链、完整 N 案例、微信小程序和用户连续操作仍未验证。演示用隔离场景入口 `db:seed:consumer-scenario` 按需创建；协作者须在自己的 PostgreSQL 中按增量执行 001—029 迁移，并按需加载 A00/B00 样例或隔离场景，不能依赖他人本机数据库状态。
...
以下是 A00 单模块手测的兼容初始化顺序，后续 consumer-a 和固定目录示例只适用于该组合；不是 M2 小程序场景初始化。M2 使用开发计划中的 consumer-scenario 入口，不应顺序叠加执行两套种子。在新隔离库从仓库根目录执行前，先核对 `.env` 指向的库：
