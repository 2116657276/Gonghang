...
| 文档字段 | 内容 |
| --- | --- |
| 更新日期 | 2026-09-12（研究资料主要于 2026-09-07 核对） |
...
| 实施状态 | 恢复开发前最近一次可引用证据覆盖 S1 本地业务基础与 S3 服务端善后基础；S2 仅完成沙箱只读查单连通性核验；Pi 0.85.1 已安装并通过本地确定性工具循环，真实 DeepSeek 只读／方案联调及局部执行评测已有记录；2026-09-11—12 已恢复开发，交易执行工具、事件恢复、T10 复核和变更善后连续链已作本地受控核对，本轮实现与检查结果以开发计划为准，S4／S5 未放行 |
...
当前通过 `https://api.deepseek.com` 的 OpenAI 兼容接口使用 DeepSeek V4.1 Flash，官方 API 标识为 `deepseek-flash`。此前公开资料与真实调用证据对应旧 `deepseek-v4-flash`；本轮已重新核对新模型标识、工具调用支持与价格，并将运行请求切换到新标识。实际接入仍须记录响应 usage、人民币费率版本、账本余额及 429 处理；本地 ¥100 是项目保护阈值，不是对平台账单的承诺。
