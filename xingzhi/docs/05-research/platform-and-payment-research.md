...
| 文档字段 | 内容 |
| --- | --- |
| 更新日期 | 2026-09-14（研究资料主要于 2026-09-07 核对） |
| 文档性质 | 外部平台、支付接口和接入限制的研究摘要 |
| 当前主线 | TypeScript 全栈网页、官方沙箱支付、Pi 单 Agent、受控计划变更与交易善后 |
| 当前模型 | Pi v0.85.1；DeepSeek V4.1 Flash，官方 API 标识 `deepseek-flash`（用户简称“ds4.1flash”）；累计模型费用上限 ¥100；常规服务端令牌桶限流 |
| 实施状态 | S1 本地业务基础与 S3 服务端善后基础已具备；S2 已取得首批沙箱付款、原单退款查询和待付关单核心链记录；Pi 0.85.1 已安装并通过本地确定性工具循环，旧模型只读／方案联调、当前模型固定场景及进程／授权连续评测已有记录；交易执行工具、事件恢复、T10 复核、变更善后连续链和 1.3／1.4 恢复准备已作本地受控核对，后端 55 项定向验收通过；官方特殊场景、多场景稳定性、前端跨端联调和 S4／S5 未放行 |
...
当前通过 `https://api.deepseek.com` 的 OpenAI 兼容接口使用 DeepSeek V4.1 Flash，官方 API 标识为 `deepseek-flash`。此前公开资料与部分真实调用证据对应旧 `deepseek-v4-flash`；本轮已重新核对新模型标识、工具调用支持与价格，并将运行请求切换到新标识。当前模型固定场景、进程中断及授权连续样本已记录；多场景稳定性、官方交易和完整业务链仍待验收。实际接入继续记录响应 usage、人民币费率版本、账本余额及 429 处理；本地 ¥100 是项目保护阈值，不是对平台账单的承诺。

## 二、平台事实与适用范围

| 方向 | 可以采用的有限结论 | 当前不作的承诺 |
| --- | --- | --- |
| 支付宝网页支付 | 公开文档提供下单、查单、关单、退款、退款查询和可选通知路径；本机采用主动查单，已取得 0.01 元付款、原单退款查询和待付关单核心链记录 | 公开接口不等于所有账号能力；费用／分批退款、异步通知和外部中断仍未完成，沙箱证据不代表正式交易 |
...
账号适用产品、付款设备和同一订单的付款—退款—关单核心链已有本机记录；通知可达性、费用／分批退款、官方外部中断和完整渠道边界仍未由本文件证明。Pi 工具事件／取消／恢复、当前模型固定场景、进程中断及授权连续样本已有本地证据，但不代表官方特殊场景或多场景用户理解效果。若公开资料与账号实际条件冲突，先把受影响能力标为阻塞，再修改[决策台账](../00-governance/decision-log.md)和[评审议题](../00-governance/review-agenda.md)，不得用模拟结果替代外部证据。
