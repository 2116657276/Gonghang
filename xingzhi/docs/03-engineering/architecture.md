...
| 文档编号 | XZ-ARCH |
| 更新日期 | 2026-09-11 |
| 状态 | 2026-09-11 已恢复开发；API-24、四个只读工具、两个方案工具、持久化运行与限流的历史切片已核对，429 切片的 36 项自动检查仍属历史证据；交易执行工具、事件恢复和页面衔接已接入并作局部行为核对，本轮实现与检查结果以开发计划 1.8 为准，S4／S5 未放行 |
...
Pi 只管理单 Agent 模型与工具循环，不与 LangGraph 双重编排；首版固定 `@earendil-works/pi-agent-core@0.85.1`、`@earendil-works/pi-ai@0.85.1` 和 DeepSeek `deepseek-v4-flash`，不直接派生通用编程助手产品。S4 接入 Pi 主机时仅注册 T01—T10 业务工具，不加载 shell、任意 HTTP、任意文件写入或支付密钥工具。选择记录见[ADR](adr/0001-pi-single-agent.md)。
...
| Agent 运行 | `routes/agent.ts`、`domain/agent-runtime.ts`、`agent-runs.ts`、`agent-tools.ts`、`business-reads.ts`、`proposals.ts`、`agent-proposals.ts`、`model-pricing.ts` | API-24、限流与超时、四个只读工具和两个方案工具已有历史验证；UI 与工具共用查询／方案服务，方案与 WAITING_USER 原子保存；真实模型方案联调通过；交易执行工具与事件恢复已接入并作局部行为核对，本轮实现与检查结果以开发计划 1.8 为准 |
...
当前实现将执行逻辑集中在 `domain/worker-runtime.ts` 与 `domain/channel-worker.ts`，HTTP 组装位于 `app.ts`；API-12、API-06R、API-25、渠道通知历史保护和领取版本检查属于此前定向核对范围。此前 31 项切片和后续 429 切片的 36 项自动检查均是历史证据；恢复开发后的交易工具、事件恢复和页面衔接以开发计划 1.8 的实现与检查结果为准，真实官方交易、S4／S5 放行仍未完成。

## 五、实现前仍需关闭的决定

首发终端、认证方案、持久数据库、后台模式和本地运行默认值已统一，S1 对应的 pnpm 工作区、Vue／Fastify、PostgreSQL 和 worker 已在本机实现。Pi 与 DeepSeek 的公开版本、模型标识和费用／限流规则已核对；Pi 包已安装且本地无资金循环通过，真实只读烟测和人民币 usage 结算已通过；交易执行工具与事件恢复仍需 S4 实现和验收。沙箱条件及渠道参数在 S2 官方调用前核验。当前不创建云服务器或固定域名部署配置；公开部署条件另行确定，详见[评审议题](../00-governance/review-agenda.md)。详细运行设计见[运行文档](../04-quality/operations.md)。
