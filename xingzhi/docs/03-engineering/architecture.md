...
Pi 只管理单 Agent 模型与工具循环，不与 LangGraph 双重编排；首版固定 `@earendil-works/pi-agent-core@0.85.1`、`@earendil-works/pi-ai@0.85.1`，当前使用 DeepSeek V4.1 Flash（官方 API 标识 `deepseek-flash`，用户简称“ds4.1flash”），不直接派生通用编程助手产品。S4 接入 Pi 主机时仅注册 T01—T10 业务工具，不加载 shell、任意 HTTP、任意文件写入或支付密钥工具。官方价格、运行准入、请求体、固定场景及进程／授权连续样本已有本地核对，多场景稳定性与官方交易仍待验收，选择记录见[ADR](adr/0001-pi-single-agent.md)。
