...
> 2026-09-17 状态：001—029、A00—A06、B00—B07 与第 11 节 M1 补充收口均已进入当前代码基线并完成集中验收。消费者 HTTP 连续链、Worker、74/74 集中验收套件、类型检查和真实模型两条代表性运行均已登记通过；同日后续 75/75 回归、构建和提交 SHA 也已完成登记，见[验证记录第十四节](docs/04-quality/verification.md)。M1 后端已放行进入 M2；微信小程序、真实银行与新流程支付宝 sandbox 连续演示仍待实施。演示数据通过第 11 节的隔离场景入口按需创建，不依赖某台开发机的固定数据状态。

更新日期：2026-09-17。本文只整理现状和开发任务，不表示本文所列业务 API 已经部署到真实银行或对外开放。M1 已完成，M2 消费者采用 Taro 4.x + Vue 3 微信小程序，实施顺序见[开发计划](docs/03-engineering/development-plan.md)。冻结的是现有业务 DTO 及 Cookie／Origin 后端链；[小程序会话增量](docs/03-engineering/api-contract.md#13-m2-小程序会话接入待实现)仍待 M2.1 实现和真机验证，恢复读取增量见同文 1.4，安排在 M2.3，不把 HTTP 通过等同于小程序已经可接入。本文同时作为 A/B 的实施分工和交接基线；遇到旧实现与本文冲突时，先保留历史交易可读与善后能力，再按本文为新消费者流程增加独立路径，不用兼容名义绕过资金准入、本人确认或来源边界。
...
| 领域 | 已完成的事实 | 仍缺少的部分 |
| --- | --- | --- |
| 数据库 | 代码基线含 001–029；020 扩展新预算周期 operation/refund 范围，027 补人工任务范围，028 增加消费者 Agent 运行范围，029 新增收口表（分类覆盖、Agent 产物、订单善后预览、隔离场景） | 未做真实银行数据审计；支付宝沙箱连续链留在 M3 |
| 历史交易 | Fastify 的旧计划/目录/订单/授权/Agent/Worker/支付宝沙盒流程存在；本机历史订单 24 条，增量迁移前后按原字段逐行一致 | 旧计划入口不用于新消费者购买；active 新资金规划用户禁止沿旧入口建单，B05/B06 已处理新预算周期订单；新消费者 Agent 不继承旧执行工具 |
| 共同契约 | `packages/contracts/src/consumer-backend.ts` 已导出严格的请求/响应校验、状态枚举和 TypeScript 类型；A00—A06、B00—B07 路由及内部端口已接通 | 前端尚未消费新契约 |
| 产品规划 | 已确定消费者目标、估价/报价分离、月度目标、逐日现金流、意外调整、安全确认和 A/B 模块界线；B02 需求／预算草稿、B03—B07 交易与恢复边界及收口 1—6（意图退出、订单独立善后、Agent 产物交接、分类展示覆盖、隔离场景）已落入 API | 前端尚未消费新契约；真实银行和新流程支付宝沙箱连续链仍未验证 |
| 验证 | 迁移器按增量应用；回滚探针验证账户/周期版本、Demo 报价过期和新旧订单边界；契约/服务端类型检查通过；集中验收套件为 74/74，同日后续补充回归为 75/75，消费者 HTTP 连续链与两条真实模型代表性检查已在隔离数据库通过 | 真实银行、前端连续流程、支付宝沙箱新消费者链和新版 N 案例仍未验证 |

2026-09-16 只读核对（029 应用前）：当时迁移登记 28 条、`orders` 24 行、`offer_quotes` 2 行、`finance_accounts`/`finance_account_snapshots`/`finance_ledger_entries`/`finance_obligations`/`budget_periods`/`budget_items`/`planning_drafts` 各 0 行；A00 资金样例尚未加载，演示资金事实用收口 5 的隔离场景入口按需创建。`huixiang` 已合并进本地 `jianlin` 基线；当前工作区的 B/M1 代码与文档变更不代表已经发布到远程 `main`，数据库实例、历史订单、样例和本地备份也不会通过 Git 自动同步，协作者仍须在各自数据库按增量执行 001—029 迁移。
...
- 历史开工输入：当时已应用 001—028、共享契约、空财务表、2 条 B00 Demo 报价和旧 seed；该段只记录 A00 开发起点。当前代码基线为 001—029，实际联调按隔离场景和最新迁移执行。
...
本节对已进入当前代码基线的 B00—B07 交付作现状记录。原开发阶段的固定桩和分批计划保留用于追溯；当前运行已接入 A02/A03、A04/A05 和 A06 的核心路径，第 11 节列出的补充收口与 HTTP 连续验收已通过。
...
| 批次 | 内容 | 允许依赖 | 统一验收结果 |
| --- | --- | --- | --- |
| B-Ⅰ | B00—B02：目录、报价、每日项目入口、只读 AI 草案 | 共享契约、A 的 `BudgetBasis`/项目写入边界；不依赖真实购买准入 | 原范围目录、草稿和 Agent 工具隔离已落地；固定 A 桩仅用于边界测试，补充收口与 HTTP 连续验收已随第 11 节收口通过 |
| B-Ⅱ | B03—B04：购买意图、本人确认、新订单、支付交接 | A04 preview、A05 同事务 commit；账户撤回错误已进入共享契约 | 新鲜 allowed 才能本人确认并建一笔订单，提交后才能支付交接，旧订单仍可读 |
| B-Ⅲ | B05—B07：Worker、资金事件、意外调整、取消退款和证据 | A06 资金事件与复盘接口 | 原范围领域链已有记录；未知结果、退款分层和完整 API 因果链已按第 11 节集中验收补齐 |
...
两人分别维护自己的领域文件；共享契约、跨表迁移和同事务函数由双方审核。分支可以各自开发，但不能各自修改已执行的 001—029 或产生相同迁移编号。对需要补字段的真实缺口追加 030+，不得重写已执行 SQL、清库或删除旧表。涉及历史订单、资金事件、授权或隐私的修改先对照本文件约束并做定向验证。

阶段 0 的历史完成判定是：双方代码基线均含 013—023 和 `consumer-backend.ts`；当前 M1 基线已追加至 029，A/B 使用同一组 DTO、日期、金额、错误码和锁顺序，各自本地数据库独立按增量应用迁移。Git 分支同步不代表 PostgreSQL 数据、历史订单、A00 资金样例或本地私密配置已经同步，任何一方都不能据此跳过自己的迁移核对。
...
7. 新业务集中验收目标套件为 74/74，同日后续补充回归为 75/75（隔离库复跑通过，见[验证记录](docs/04-quality/verification.md)第十四节）；模型费用账本、旧 Agent 全局队列计数等共享状态的隔离修复已随本次复跑确认。

## 10. 已有实现与恢复入口

第 11 节收口 1—6 已于 2026-09-16 实施并通过集中验收，M2 前端可按冻结契约开工。数据库结构、A00—A06、B00—B07、共享字段定义和 029 收口迁移已经进入当前代码基线。新消费者 Agent 已接入实际运行，但工具仍限定为本人资金读取、登记商品检索和非执行草稿保存。B03 的 `proposed` 意图必须经过 B04 本人确认，B05/B06 的渠道及资金事实继续遵守 A06 到账证据边界；任何阶段都不能越过资金、确认或支付边界。

## 11. M1 后端收口：进入 M2 前的补充任务

2026-09-16 只读审核撤回了“后端契约已冻结、直接进入 M2”的放行结论并设立本节补充收口；同日收口 1—6 已实施，并按 [验证策略第十四节](docs/04-quality/verification.md)完成集中验收（HTTP 连续链、Worker 恢复、幂等权限、Agent 交接、财务展示与隔离场景；74/74 目标套件与真实模型代表性检查通过）。本节因此从“待实施契约”转为已落地的收口记录，以下小节保留当时的任务定义与验收要求，实际接口以 [API 契约](docs/03-engineering/api-contract.md)与共享契约代码为准。仍属于 M1，不另建大阶段或第二套文档。

顺序固定为：收口 1 → 收口 2 → 收口 3 → 收口 4 → 收口 5 → 收口 6。B 主责交易、Agent、HTTP 接线和整合；A 主责资金事实、分类展示数据及 Demo 账户数据；共享契约和新增迁移由实际执行者统一落笔，沿用同一 `PoolClient`，不得各自创建同号迁移。单人继续 B 工作时，可以按本节修改必要的 A 公共模块，无需重复实现资金算法。已执行迁移（现为 001—029）不改写，只在确有字段需求时追加新编号迁移。
...
- 在创建意图以及编辑/取消同一项目时，按账户→周期→项目→意图的既有顺序，在业务事务内先把到期的 proposed 意图转为 expired，再检查有效意图。失效仅释放意图占用，不增加账户余额、不释放已建单承诺；这类惰性清理足够，不新增定时扫描服务。事务回滚后，下次调用仍须能够重复清理，不能依赖失败请求曾经保存状态。放弃入口与确认入口采用完全一致的锁序串行化：同一意图并发确认与放弃时只有一个状态迁移成功，另一方得到 409 业务冲突，不产生数据库死锁或 500（已由并发回归覆盖）。
...
收口 1—5 完成后统一验收，具体范围见 [验证策略第十四节](docs/04-quality/verification.md#十四m1-后端收口集中验收通过)；该验收已通过，新 DTO、路径及样例已冻结为 M2 基线。69/69 历史记录已由隔离库 74/74 复跑取代，不能代替收口新增行为的验收。无须逐任务全量测试或构建；已有实现未变的证据复用，资金/授权问题只在需要定位时做窄范围复现。
