...
> 2026-09-16 实施更新：001—029、A00—A06、B00—B07 与第 11 节 M1 补充收口均已进入当前代码基线并完成集中验收：消费者 HTTP 连续链、Worker、74/74 目标套件、类型检查和真实模型两条代表性运行（有账户/无账户）均已在隔离数据库登记通过，M1 后端放行进入 M2；剩余登记动作（构建结果、提交 SHA）见 [验证记录第十四节](docs/04-quality/verification.md)。统一前端（M2）、真实银行与新流程支付宝沙箱连续演示（M3）仍待实施。当前 `xingzhi_dev` 的 A00 资金样例仍未加载，演示使用第 11 节收口 5 的隔离场景入口，详细证据见 [验证记录](docs/04-quality/verification.md)。
...
| 验证 | 迁移器按增量应用；回滚探针验证账户/周期版本、Demo 报价过期和新旧订单边界；契约/服务端类型检查通过；目标套件最近记录 74/74（含收口新增 5 组），消费者 HTTP 连续链与两条真实模型代表性检查已在隔离数据库通过 | 真实银行、前端连续流程、支付宝沙箱新消费者链和新版 N 案例仍未验证 |
...
7. 新业务的定向测试和目标套件最近记录为 74/74（含收口新增 5 组，隔离库复跑通过，见[验证记录](docs/04-quality/verification.md)第十四节）；模型费用账本、旧 Agent 全局队列计数等共享状态的隔离修复已随本次复跑确认。
