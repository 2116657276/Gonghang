...
const pendingActions = computed<PendingAction[]>(() => {
  ...
  for (const run of agentRuns.value.filter(value => value.state === 'RUNNING')) values.push({ id: `agent-${run.id}`, title: '行止正在整理你的问题',
    detail: '可查看进度或停止运行', url: '/pages/ai/history', tone: 'info', preference: 'planning' });
  return values;
});
...
const visibleDaily = computed(() => expandedDaily.value ? rollingDaily.value : rollingDaily.value.slice(0, 7));
    ...
    <template v-else>
      <view class="summary-grid">
        <SectionCard class="summary-card" @tap="openAccount">
          <view class="summary-card__heading"><text class="summary-card__label">我的主账户</text><button class="visibility-button" :aria-label="balanceVisible?'隐藏主账户金额':'显示主账户金额'" @tap.stop="toggleBalanceVisibility">{{ balanceVisible ? '隐藏' : '显示' }}</button></view>
          ...
          <text class="summary-card__foot">{{ overview.primaryAccount.value?.cashBasis.dataStatus==='observed'?'已核验可用现金':'资金依据待完善' }} · {{ factTime }}</text>
        </SectionCard>
        <SectionCard class="summary-card" @tap="activeSheet='impact'">
          ...
          <text class="summary-card__amount amount">{{ yuan(rollingForecast?.minimumProjectedCashMinor) }}</text>
          <text class="summary-card__foot">{{ rollingFoot }} · {{ shortDate(rollingForecast?.minimumCashOn) }}</text>
        </SectionCard>
      </view>
      ...
      <SectionCard v-if="rollingDaily.length" class="forecast-card">
        <text class="forecast-card__summary">{{ cashflowSummary }}</text>
        <text class="forecast-card__note">逐日预测只使用已确认现金和已登记安排；预计收入不提前计入现金。</text>
        <view class="daily-list" aria-label="未来逐日资金预测">
          <view v-for="day in visibleDaily" :key="day.on" class="daily-row" :class="{'daily-row--unknown':day.dataStatus==='unknown','daily-row--minimum':day.on===rollingForecast?.minimumCashOn}">
            <text>{{ shortDate(day.on) }}</text><text>{{ day.dataStatus==='unknown' ? '依据不足，趋势中断' : yuan(day.projectedCashMinor) }}</text>
          </view>
        </view>
        <button v-if="rollingDaily.length>7" class="text-action" @tap="expandedDaily=!expandedDaily">{{ expandedDaily ? '收起日期' : `查看全部 ${rollingDaily.length} 天` }}</button>
      </SectionCard>
...
.forecast-error{margin-top:14px}.forecast-card__summary,.forecast-card__note,.minimum-card__title,.minimum-card__note{display:block}.forecast-card__summary{font-size:25px;font-weight:700;line-height:1.5}.forecast-card__note,.minimum-card__note{margin-top:8px;color:$text-secondary;font-size:20px;line-height:1.5}.daily-list{margin-top:20px;border-top:1px solid $border}.daily-row{display:flex;justify-content:space-between;gap:16px;padding:13px 0;border-bottom:1px solid $border;font-size:22px}.daily-row text:last-child{font-weight:650}.daily-row--minimum{color:$brand-deep}.daily-row--unknown{color:$text-tertiary;border-bottom-style:dashed}.daily-row--unknown text:last-child{font-weight:400}.text-action{width:100%;margin-top:14px;padding:12px;color:$brand-primary;background:transparent;font-size:21px}.minimum-card{margin-top:14px;background:$soft-surface}.minimum-card__title{font-size:23px;font-weight:700}.cause-list{display:grid;gap:7px;margin-top:10px;color:$text-secondary;font-size:21px}.timeline-card{padding-block:8px}.timeline-row{display:flex;align-items:flex-start;gap:14px;padding:17px 0}.timeline-row+.timeline-row{border-top:1px solid $border}.timeline-row__date{flex:0 0 90px;color:$text-secondary;font-size:20px}.timeline-row__copy{flex:1;min-width:0}.timeline-row__copy text{display:block;font-size:22px;line-height:1.35}.timeline-row__copy text+text{margin-top:5px;color:$text-tertiary;font-size:18px}.timeline-row__amount{font-size:21px;font-weight:680;white-space:nowrap}
