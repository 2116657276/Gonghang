...
.login-page { position:relative; min-height:100vh; padding:var(--app-safe-top, calc(64px + env(safe-area-inset-top))) 34px calc(40px + env(safe-area-inset-bottom)); overflow-x:hidden; overflow-y:auto; background:linear-gradient(180deg,#E6EFEA 0%,#F4F7F5 60%); }
...
.login-brand__tagline { margin-top:10px; color:$text-secondary; font-size:28px; }
...
.login-card__title { display:block; font-size:36px; font-weight:720; }.login-card__hint { display:block; margin-top:8px; color:$text-secondary; font-size:27px; }
.field { display:block; margin-top:28px; color:$text-secondary; font-size:26px; }.field input { height:88px; margin-top:10px; padding:0 24px; color:$text-primary; background:$soft-surface; border:1px solid $border; border-radius:18px; font-size:30px; }
.login-card .notice { margin-top:22px; }.login-submit { width:100%; margin-top:30px; }.login-footnote { display:block; margin-top:24px; color:$text-tertiary; font-size:25px; line-height:1.55; text-align:center; }
@media screen and (max-height:700px){.login-brand :deep(.ip-avatar--large){transform:scale(.78);margin-block:-18px}.login-brand__name{margin-top:8px;font-size:44px}.login-brand__tagline{margin-top:4px}.login-card{margin-top:22px;padding:28px 26px}.field{margin-top:20px}.field input{height:76px}.login-footnote{margin-top:18px}}
</style>
