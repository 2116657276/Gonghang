...
<template>
  <PageShell title="我的" subtitle="管理账户、授权和行止设置">
    ...
    <template v-else>
      <SectionCard class="identity-card" @tap="Taro.navigateTo({url:'/subpackage/common/profile-info'})"><IpAvatar size="medium"/><view class="identity-card__copy"><text>{{ overview.user.value?.displayName }}</text><text>{{ overview.user.value?.email }}</text></view><view class="identity-card__action"><text>查看资料</text><text aria-hidden="true">›</text></view></SectionCard>
      <SectionCard class="account-card" @tap="openAccount"><view class="row-between"><view><text class="account-card__title">我的主账户</text><text class="account-card__name">{{ displayAccount?.account.displayName??'尚未连接账户' }} {{ displayAccount?.account.maskedIdentifier??'' }}</text><text v-if="displayAccount" class="account-card__source">{{displayAccount.account.source==='demo'?'Demo 数据':'授权账户'}} · {{accountStatus}}</text></view><text class="account-card__status">{{accountStatus}}</text></view><view class="account-card__balance"><text class="account-card__amount amount">{{ !overview.primaryAccount.value?'—':balanceVisible?yuan(overview.primaryAccount.value.cashBasis.confirmedCashMinor):'••••' }}</text><button v-if="overview.primaryAccount.value" class="visibility-button" :aria-label="balanceVisible?'隐藏主账户金额':'显示主账户金额'" @tap.stop="toggleBalanceVisibility">{{balanceVisible?'隐藏':'显示'}}</button></view><view class="account-card__foot"><text>{{ accountUpdated }}</text><text>{{displayAccount?'查看依据':'选择账户'}} ›</text></view></SectionCard>

      <view class="setting-list">
        <SettingGroup title="规划设置" icon="planning" :open="true">
        ...
        <SettingGroup title="数据与授权" icon="data" :open="true">
        ...
        <SettingGroup title="应用设置" icon="app" :open="true">
        ...
        <SettingGroup title="历史与复盘" icon="history" :open="true">
        ...
        <SettingGroup title="账户管理" icon="account" :open="true">
...
.identity-card{min-height:128px;gap:24px;background:$surface-tint;border-color:rgba(38,125,98,.12)}.identity-card__copy text{font-size:40px;font-weight:700;line-height:1.35}.identity-card__copy text+text{margin-top:6px;font-size:28px;font-weight:400;overflow-wrap:anywhere;white-space:normal}.identity-card__action{display:flex;flex:0 0 auto;align-items:center;gap:8px;color:$brand-primary}.identity-card__action text{font-size:28px}.identity-card__action text:last-child{font-size:38px}.account-card{margin-top:24px;background:linear-gradient(145deg,$surface-tint 0%,$card 125%);border-color:rgba(38,125,98,.14)}.account-card__title{font-size:36px;font-weight:600}.account-card__name{font-size:28px}.account-card__source{display:block;margin-top:8px;color:$text-tertiary;font-size:26px}.account-card__status{flex:0 0 auto;padding:8px 12px;color:$brand-deep;background:rgba(255,255,255,.72);border-radius:999px;font-size:26px}.account-card__balance{margin-top:28px}.account-card__amount{font-size:64px;font-weight:700;line-height:1.2;overflow-wrap:anywhere}.visibility-button{min-width:88px;min-height:72px;padding:8px 14px;color:$brand-primary;background:rgba(255,255,255,.8);font-size:26px}.account-card__foot{margin-top:16px;font-size:26px;line-height:1.5}.setting-list{margin-top:24px}.setting-row{min-height:112px;gap:24px;padding:20px 0}.setting-row>text{flex:1;min-width:0;font-size:32px;font-weight:500}.setting-row view{display:grid;grid-template-columns:minmax(0,1fr) 32px;flex:0 1 52%;gap:12px}.setting-row view text:first-child{max-width:none;overflow:visible;font-size:28px;line-height:1.45;text-overflow:clip;white-space:normal;text-align:right}.setting-row view text:last-child{align-self:center;font-size:36px;text-align:right}.logout-button{margin-top:32px;background:$card;border-color:rgba(175,57,65,.22)}.confirm-note{padding:24px 28px;border-radius:24px;font-size:28px}.confirm-note--warning{color:$warning}
@media screen and (max-width:360px){.identity-card{align-items:center;gap:18px}.identity-card__copy text{font-size:36px}.identity-card__action text:first-child{display:none}.account-card__amount{font-size:56px}.account-card__foot{align-items:flex-start}.setting-row{gap:16px}.setting-row>text{flex:1}.setting-row view{flex-basis:54%}}
</style>
