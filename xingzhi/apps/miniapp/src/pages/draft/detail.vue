...
import type { BudgetItemChangePreview, BudgetItemInput, PlanningDraft } from '@/lib/types';
...
function adopt(index: number) {
  if (!draft.value?.periodId || !['draft', 'accepted'].includes(draft.value.status)) return;
...
async function showImpact(index: number) {
  ...
  if (!value?.periodId || !item) return;
  if (!complete(index)) { adopt(index); return; }
  if (value.basisFinancialVersion === null || value.basisPeriodVersion === null
    || !item.plannedOn || !item.userEstimatedAmountMinor || !item.priority) return;
  impactLoading.value = index; impactError.value = '';
  const input: BudgetItemInput = {
    periodId: value.periodId, itemId: null,
    expectedFinancialVersion: value.basisFinancialVersion,
    expectedPeriodVersion: value.basisPeriodVersion,
  ...
  try { impact.value = (await api.budgetItemChangePreview(value.periodId, input)).data;
