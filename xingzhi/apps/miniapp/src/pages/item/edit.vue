...
useLoad(async options => {
  ...
  try {
    ...
    } else if (options.draftId && options.index !== undefined) {
...
<style lang="scss">@use '../../styles/tokens' as *;.back{position:absolute;z-index:4;top:calc(34px + env(safe-area-inset-top));right:28px;width:64px;height:64px;color:$brand-deep;background:rgba(255,255,255,.78);border-radius:50%;font-size:44px}.error-box{margin-bottom:20px}.form-card{padding-top:36px;padding-bottom:36px}.field{display:block}.field+.field{margin-top:38px}.field>text{display:block;margin-bottom:14px;color:$text-secondary;font-size:28px;font-weight:620}.field input,.field textarea,.picker-value{width:100%;padding:0 24px;background:$soft-surface;border:2px solid $border;border-radius:22px;font-size:30px}.field input,.picker-value{height:96px;line-height:96px}.field textarea{min-height:168px;padding-block:22px;line-height:1.55}.choices{display:flex;flex-wrap:wrap;gap:14px}.choices button{flex:1 1 180px;min-height:88px;padding:14px;color:$text-secondary;background:$soft-surface;border:2px solid $border;border-radius:20px;font-size:28px}.choices .choice--active{color:$brand-primary;background:$surface-tint;border-color:#AFCBBB}.choices button[disabled]{opacity:.42}.save{width:100%;margin-top:28px}.preview-note{margin-top:22px;font-size:28px;line-height:1.6}.danger-link{display:block;min-height:88px;margin:24px auto;color:$danger;font-size:28px}</style>
