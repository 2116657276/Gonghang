...
export async function readCancellationQuote(user: AuthUser, id: string) {
    const result = await query<{ id: string; owner_id: string; amount_minor: number; payment_status: string; cancellation_rule: CancellationRule; rule_version: number }>(`
      SELECT orders.id, orders.owner_id, orders.amount_minor, orders.payment_status, catalog_items.cancellation_rule, catalog_items.rule_version
