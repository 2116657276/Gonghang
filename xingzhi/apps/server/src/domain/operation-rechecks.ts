...
async function relatedOrderForOperation(client: PoolClient, type: string, entityId: string) {
  if (['simulate_payment', 'simulate_close', 'sandbox_payment_handoff', 'sandbox_payment_recheck', 'sandbox_close'].includes(type)) {
    const result = await client.query<{ order_id: string; merchant_id: string }>(
      'SELECT id AS order_id, merchant_id FROM orders WHERE id = $1', [entityId],
    );
    return result.rows[0];
  }
  if (['simulate_refund', 'merchant_cancellation_review'].includes(type)) {
    const result = await client.query<{ order_id: string; merchant_id: string }>(`
      SELECT cancellation_requests.order_id, orders.merchant_id
...
export async function requestOperationRecheck(
  ...
  const planRef = await client.query<{ plan_id: string }>('SELECT plan_id FROM operations WHERE id = $1', [operationId]);
  const planId = planRef.rows[0]?.plan_id;
  if (!planId) notFound('未找到可复核的操作。');
  await client.query('SELECT id FROM plans WHERE id = $1 FOR UPDATE', [planId]);
  const operationResult = await client.query<{ id: string; plan_id: string; owner_id: string; type: string; entity_id: string }>(
    'SELECT id, plan_id, owner_id, type, entity_id FROM operations WHERE id = $1 FOR UPDATE', [operationId],
  );
  ...
  const task = await ensureManualTask(client, {
    dedupeKey: `operation-recheck:${operation.id}`, planId: operation.plan_id, merchantId: order.merchant_id,
