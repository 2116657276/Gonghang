...
仅在首次初始化全新、隔离的演示数据库时使用 `--seed`/`-Seed`。该选项会运行通用 `db:seed`，重置已有商户/审核测试账号的密码哈希并清除登录失败限制；不要对当前已有 Demo 库、共享库或日常开发库重跑，也不要通过删除业务数据绕过保护。
...
导入前需通过负责人提供的私下渠道取得本机 `project.config.json`，并确认其中配置了有权使用的 AppID 和 `miniprogramRoot: dist/weapp/`；不要把该文件提交到 Git。在微信开发者工具中导入 `xingzhi/apps/miniapp`，不要直接导入 `dist/weapp`。本机开发者工具可以使用 `http://127.0.0.1:8877`；Windows 热点真机调试可使用上面的 `-Mobile`。正式预览和发布仍必须使用手机可访问的 HTTPS API 和已配置的微信合法域名。
...
# 小程序页面脚本回归（不含实际 UI 渲染）
pnpm --filter @xingzhi/miniapp test:pages
...
仓库包含 `001` 至 `036` 的迁移文件；`034` 增加登录失败限制和 AI 账目提问月份，`035` 增加普通流水与预算计划的关联/解除记录，`036` 允许撤回账户历史恢复所需的周期版本更新，仍限制规划字段变更。文件存在不等于目标数据库已经执行；接手或换机器时要核对目标库 `schema_migrations`，不要手工改表、回写已执行 SQL 或把 Git 合并当作迁移完成。

Simulation 是本地业务链证据，支付宝 Sandbox 是独立渠道环境，两者不能互相替代。付款、取消和退款都必须保留用户确认、权限、幂等、原业务号和恢复边界；渠道成功不等于银行账户已到账。

## 7. 当前阶段

本地八项跨模块修复、远程 `huixiang` 的 S1—S5 UI/交互源码改动及 S6 代码级回归相关内容已在本地合并提交 `fb5095d`。验证按批次区分：本地修复阶段服务端 92/92；远程分支的测试、模型调用、热点地址和包体数字是远程开发者当时的记录；合并时页面脚本 12/12、四工作区类型检查及 H5/微信/管理端构建通过。合并后测试角色又按现有入口完成服务端 92/92、小程序两份脚本 15/15（这两套合计 107/107），并通过小程序类型检查和差异格式检查。该次后续运行没有构建或验收实际页面；S6 最终人工验收、微信开发者工具、真机、当前环境模型和支付宝 Sandbox 仍待验收。迁移 036 已在此前本地修复阶段应用于本机 Simulation 库，其他数据库仍须核对登记。下一步在微信开发者工具重新编译、分析包体并执行财务主线，再做真机最小主线和财务 Demo；交易专项按交付范围另行验收。详细状态与唯一顺序见 [`04-development.md`](xingzhi/doc/04-development.md)，操作见[开发接力指南](开发接力指南.md)。
